...
down = np.asarray([], dtype=str)
up = np.asarray([], dtype=str)
neut = np.asarray([], dtype=str)
#get the data without the first column as integers
bar = progressbar.ProgressBar()
def trial(n_trails):
    for trial in bar(range (0, n_trails)):
        filename='DE0.csv'
